...
switch output
    ...
    case 'pipeline'
        ...
        plot(xi,yi,'--r')
        line(xlim,min(yi).*[1 1],'linestyle','-.')
        line(xlim,max(yi).*[1 1],'linestyle','-.')
        hold off
        clear out xi yi
end
...
%         yi = interpolate(yi,out,1);
        ...
        function refit
            Ylim                = [min(yi) max(yi)];
            X                   = zeros(size(out,1),1);
            X(Ylim(1):Ylim(2))  = imresize(xi,[diff(Ylim)+1 1]);
            Y                   = linspace(0,length(X),length(X));
        end
